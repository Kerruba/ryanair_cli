...
function destinations(from, opt) {
  ...
  api
    ...
    .then(res => {
      ...
      let _airport = airports.filter(val =>
        utilities.strEqualIgnoreCase(val.iataCode, from)
      );
      if (_airport.length == 1) {
        let destinations = _airport[0].routes
          .filter(route => route.startsWith("airport"))
          .reduce((prev, curr) => {
            let [, val] = curr.split(":");
            let _dest = airports.filter(air => air.iataCode === val)[0];
            let city = results.cities
              .filter(_city => _city.code === _dest.cityCode)
              .map(_city => _city.name)[0];

            let country = results.countries
